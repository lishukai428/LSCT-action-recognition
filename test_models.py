...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        ensembled_predict = sum(this_rst0_list) / len(this_rst0_list)
...
video_pred = [np.argmax(x[0]) for x in output0]
video_pred_top5 = [np.argsort(np.mean(x[0], axis=0).reshape(-1))[::-1][:5] for x in output0]
